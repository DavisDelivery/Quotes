...
let _pfxCache = null, _pfxFor = null;
function knownPrefixes(m) {
  if (_pfxFor !== m) { const s = new Set(Object.keys(m.zone3 || {})); for (const z of Object.keys(m.zone5)) s.add(z.slice(0, 3)); _pfxCache = s; _pfxFor = m; }
  return _pfxCache;
}
// Distance (mi) from the depot to a ZIP centroid, or null if we have no centroid.
export function zipDistance(zip, geo = G, origin = M.origin) {
  const c = geo[String(zip).slice(0, 5)];
  return (c && origin) ? haversine(origin, c) : null;
}
// Resolve a shipment to a de-fueled linehaul + a confidence tier:
//   data   - ZIP is in the Uline billing data (data-backed)
//   nearby - ZIP not in data but a neighboring prefix is, within the envelope
//   oa     - out-of-area: beyond the data; distance-scaled estimate (flagged)
function quoteCore(m, { zip, w, sk, lo, distMi, oaPerMile }) {
  const z = String(zip);
  const radius = m.dataRadiusMi ?? 80;
  const inData = z in m.zone5;
  const pfxKnown = knownPrefixes(m).has(z.slice(0, 3));
  const out = !inData && (!pfxKnown || (distMi != null && distMi > radius));
  if (!out) {
    const zb = zbaseM(m, zip);
    return { tier: inData ? "data" : "nearby", zb, lh: spineM(m, zb, w) + residM(m, zb, sk, lo, w), distMi, oaExtra: 0, radius };
  }
  const edge = (m.zoneMin && m.zoneMin.Z3) || 71.58;          // farthest data-backed zone
  const perMi = (oaPerMile != null ? oaPerMile : (m.oaPerMile ?? 1.75));
  const beyond = (distMi != null) ? Math.max(0, distMi - radius) : 0;
  const oaExtra = perMi * beyond;                              // de-fueled distance premium
  return { tier: "oa", zb: edge, lh: spineM(m, edge, w) + residM(m, edge, sk, lo, w) + oaExtra, distMi, oaExtra, radius };
}

// Pure quote function — exported so host apps can price without the UI.
export function priceQuote(model, { zip, weight, skids = 1, loose = 0, fuelPct = 28, marginPct = 0, accessorial = 0, distMi = null }) {
  const w = Number(weight) || 0, f = (Number(fuelPct) || 0) / 100, p = (Number(marginPct) || 0) / 100;
  const c = quoteCore(model, { zip, w, sk: skids, lo: loose, distMi: distMi != null ? distMi : zipDistance(zip) });
  const allIn = c.lh * (1 + f);
  const acc = Number(accessorial) || 0;
  const freight = allIn * (1 + p);
  return { zone: ZN[Math.round(c.zb * 100) / 100] || "Z?", zb: c.zb, tier: c.tier, distMi: c.distMi, oaExtra: c.oaExtra, linehaul: c.lh, fuelAmt: c.lh * f, allIn, accessorial: acc, freight, quoted: freight + acc };
}
...
const distOf = (zip) => { const c = G[String(zip).slice(0, 5)]; return (MODEL.origin && c) ? haversine(MODEL.origin, c) : null; };
const compute = (zip, w, sk, lo) => quoteCore(MODEL, { zip, w, sk, lo, distMi: distOf(zip) });

function confidence(tier, tp, weight, loose, distMi) {
  if (tier === "oa") return { key: "oa", label: "out-of-area", note: `Outside our Uline data${distMi != null ? ` (~${Math.round(distMi)} mi from the depot)` : ""} — this is a distance-based estimate, not a data-backed rate. Confirm before quoting firm.` };
  if (tier === "nearby") return { key: "est", label: "nearby estimate", note: "This ZIP isn't in the data, but a neighboring area is — rate uses the nearby zone." };
...
export default function UlineQuoteConsole({ model, modelUrl, initialZip, initialWeight, initialSkids, initialLoose, embedded } = {}) {
  ...
  const q = useMemo(() => {
    ...
    const allIn = c.lh * (1 + f);
    const freight = allIn * (1 + p);
    const accAmt = ACCESSORIALS.reduce((s, a) => s + (acc[a.key] ? a.amount : 0), 0);
    const quoted = freight + accAmt;
    const tp = Math.max(skids + loose, 1);
    return { zb: c.zb, lh: c.lh, fuelAmt: c.lh * f, allIn, freight, accAmt, quoted, marginAmt: freight - allIn, tier: c.tier, distMi: c.distMi, oaExtra: c.oaExtra, radius: c.radius, conf: confidence(c.tier, tp, w, loose, c.distMi) };
  }, [zip, weight, skids, loose, fuel, margin, modelTick, acc]);
  ...
  const confTone = q.conf.key === "core" ? "ok" : (q.conf.key === "est" || q.conf.key === "oa") ? "warn" : "";
  ...
  return (
    <div className={"rc-root" + (embedded ? " rc-embedded" : "")} style={accVars}>
      ...
      <div className="rc-wrap">
        ...
        {tab === "quote" && (
          <>
            {(q.tier === "oa" || q.tier === "nearby") && (
              <div className="rc-alert">
                <span className="rc-alerticon">!</span>
                {q.tier === "oa" ? (
                  <div><b>Out-of-area quote.</b> {q.distMi != null ? `~${Math.round(q.distMi)} mi from the depot — ` : ""}beyond our Uline data. This is a distance-based estimate, not a data-backed rate.</div>
                ) : (
                  <div><b>Out-of-area quote.</b> This ZIP isn't in our Uline data — priced from the nearest known zone. Confirm before quoting firm.</div>
                )}
              </div>
            )}
            <div className="rc-hero">
              ...
              <div className="rc-hstatus">
                ...
                <span className="rc-hpill">{q.tier !== "data" ? "out-of-area" : "zone " + zoneLabel(q.zb)}</span>
                {q.distMi != null && <span className="rc-hpill">{Math.round(q.distMi)} mi</span>}
            ...
            <div className="rc-card">
              ...
              <div className="rc-ledger">
                ...
                {q.tier === "oa" && q.oaExtra > 0 && (
                  <LedgerRow k={`incl. distance · +${Math.round(Math.max(0, q.distMi - q.radius))} mi`} v={"$" + money(q.oaExtra * (1 + (Number(fuel) || 0) / 100))} />
                )}
                ...
                {ACCESSORIALS.filter((a) => acc[a.key]).map((a) => (
                  <LedgerRow key={a.key} k={a.label} v={"+$" + money(a.amount)} />
                ))}
              ...
              <div className="rc-form">
                ...
                <Field full label="Accessorials" hint="Flat fees added to the quote">
                  <div className="rc-accs">
                    {ACCESSORIALS.map((a) => (
                      <button key={a.key} className={"rc-acc" + (acc[a.key] ? " on" : "")} onClick={() => setAcc((s) => ({ ...s, [a.key]: !s[a.key] }))} aria-pressed={!!acc[a.key]}>
                        {a.label} <span className="rc-accamt">+${a.amount}</span>
                      </button>
                    ))}
                  </div>
                </Field>
